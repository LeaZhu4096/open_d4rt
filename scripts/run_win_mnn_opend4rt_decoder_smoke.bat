...
set "MEMORY_TOKENS=%~6"
set "HIDDEN_DIM=%~7"
if "%MODEL_PATH%"=="" set "MODEL_PATH=%REPO_ROOT%\artifacts\mnn_vulkan\opend4rt_32clip_decoder_q8.mnn"
if "%NUM_FRAMES%"=="" set "NUM_FRAMES=2"
if "%HEIGHT%"=="" set "HEIGHT=32"
if "%WIDTH%"=="" set "WIDTH=32"
if "%NUM_QUERIES%"=="" set "NUM_QUERIES=8"
if "%MEMORY_TOKENS%"=="" set "MEMORY_TOKENS=4"
...
cl /nologo /EHsc /std:c++17 ^
    /I"%MNN_ROOT%\include" ^
    /I"%MNN_ROOT%\source" ^
    /Fo:"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_decoder_smoke.obj" ^
    "%REPO_ROOT%\scripts\mnn_opend4rt_decoder_smoke.cpp" ^
    /Fe:"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_decoder_smoke.exe" ^
    /link /LIBPATH:"%MNN_ROOT%\build_win_vulkan" MNN.lib
if errorlevel 1 exit /b 1

set "PATH=%MNN_ROOT%\build_win_vulkan;%PATH%"

echo === OpenD4RT decoder CPU control ===
"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_decoder_smoke.exe" "%MODEL_PATH%" 0 %NUM_FRAMES% %HEIGHT% %WIDTH% %NUM_QUERIES% %MEMORY_TOKENS% %HIDDEN_DIM%
if errorlevel 1 exit /b 1

echo === OpenD4RT decoder Vulkan iGPU ===
"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_decoder_smoke.exe" "%MODEL_PATH%" 7 %NUM_FRAMES% %HEIGHT% %WIDTH% %NUM_QUERIES% %MEMORY_TOKENS% %HIDDEN_DIM%
if errorlevel 1 exit /b 1
