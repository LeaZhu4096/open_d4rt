...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " decoder.mnn [forward_type] [num_frames] [height] [width] [num_queries] [memory_tokens] [hidden_dim]\n";
        return 2;
    }
    ...
    const int memoryTokens = argc >= 8 ? std::atoi(argv[7]) : 4;
    const int hiddenDim = argc >= 9 ? std::atoi(argv[8]) : 1280;
    ...
    const std::map<std::string, std::vector<int>> shapes = {
        {"memory", {1, memoryTokens, hiddenDim}},
    ...
    try {
        fillFloatInput(net.get(), session, "memory", 0.001f);
    ...
    std::cout << "MNN OpenD4RT decoder smoke completed with forward_type=" << static_cast<int>(forwardType) << "\n";
    return ok ? 0 : 1;
}