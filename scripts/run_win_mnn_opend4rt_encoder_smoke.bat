...
set "HEIGHT=%~3"
set "WIDTH=%~4"
if "%MODEL_PATH%"=="" set "MODEL_PATH=%REPO_ROOT%\artifacts\mnn_vulkan\opend4rt_32clip_encoder_t2_32.mnn"
...
cl /nologo /EHsc /std:c++17 ^
    /I"%MNN_ROOT%\include" ^
    /I"%MNN_ROOT%\source" ^
    /Fo:"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_encoder_smoke.obj" ^
    "%REPO_ROOT%\scripts\mnn_opend4rt_encoder_smoke.cpp" ^
    /Fe:"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_encoder_smoke.exe" ^
    /link /LIBPATH:"%MNN_ROOT%\build_win_vulkan" MNN.lib
if errorlevel 1 exit /b 1

set "PATH=%MNN_ROOT%\build_win_vulkan;%PATH%"

echo === OpenD4RT encoder CPU control ===
"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_encoder_smoke.exe" "%MODEL_PATH%" 0 %NUM_FRAMES% %HEIGHT% %WIDTH%
if errorlevel 1 exit /b 1

echo === OpenD4RT encoder Vulkan iGPU ===
"%REPO_ROOT%\artifacts\mnn_vulkan\mnn_opend4rt_encoder_smoke.exe" "%MODEL_PATH%" 7 %NUM_FRAMES% %HEIGHT% %WIDTH%
if errorlevel 1 exit /b 1
